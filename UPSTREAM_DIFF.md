...
| `pyproject.toml` | 依赖、项目 URL 与 Hatch wheel force-include | 安装与构建入口 | 声明代码直接导入的 `regex` 与离线 wheel 门禁所需的 Hatchling 开发依赖，把项目元数据指向 fork，并将根目录配置和提示词映射到 wheel 内 `babeldoc/_resources` | C16 |
...
| `.github/workflows/checks.yml` | `checks` job | CI 轻量门禁 | 保留 compileall、Ruff 和 wheel 构建，增加源码/安装 wheel 外部目录启动烟雾及 C01-C16 fast 规格，解析烟雾不再传入伪 key | C16 |
