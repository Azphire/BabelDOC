...
10. 性能：阶段计时汇总 → b11 靶单。
11. W-B10-13/14 加入文档迁移（迁移清单共六条）;
12. 共享 form 跨页耦合入 gap register + 失败分类账（"共享资源别名"类,column_reflow 的锚三为现成检测器）;
13. F3 运行协议:先做一次全量预热跑（消化 AW 术语抽取首见请求及其六页级联）,阶段计时以温缓存臂为准,预热臂计时单独留档标注;
14. Courier-en 缓存温度差（378s→55s）作为 b11 性能靶单的第一条实测输入。
