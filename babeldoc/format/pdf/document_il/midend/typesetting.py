...
class Typesetting:
    ...
    def glyph_ink_metrics(self, character) -> GlyphInkMetric | None:
        ...
        if len(text) != 1 or not font_id or font_size is None:
            return None
        try:
    ...
    def _layout_typesetting_units(
        ...
        # A functional avoidance carries its own measured width: the source's
        # first line was clearing printed ink, and four space widths of the
        # target font know nothing about where that ink ends. It applies
        # whatever the indent flag says, because the repair loop may order a
        # clearance for a paragraph the source set flush; stylistic indents
        # keep the flag-gated em approximation.
        clearance = functional_clearance_width(self.translation_config, paragraph)
        if clearance is not None:
            current_x += clearance
        elif paragraph.first_line_indent:
