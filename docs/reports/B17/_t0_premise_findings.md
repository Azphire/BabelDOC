...
| P3 | **不符** | issues.after.json 有 **6 条 `untranslated_residue`**(非零):`p2#16 遗症开始`、`p3#4 编者的话`、`p5#55 年`、`p5#7 The条毫无二致。`、`p6#33 低`、`p6#34 高`,全部 severity high、`min_script_chars=1`。**检测地板分方向**(执行中修正的初判):`residue_min_script_chars_into_en=1` / `into_zh=12`——into_en 方向短带已存在且在 fd-zh 上全命中;真实检测缺口仅在 into_zh 方向。12 字修复地板另在修复准入:termination.json untranslated_residue 决策 no_op,理由 "none have at least 12 source characters with a residue ratio of at least 0.9"(`translate_orphan_text` 条件,[minimal_repair.py:787](../../babeldoc/magazine/minimal_repair.py#L787) `orphan_min_source_chars`)。T1d 改为对 into_zh 缺口离线重放定标,测量结论见 [t1d_short_band_replay.md](t1d_short_band_replay.md):零真实漏译、两样本超预算,**地板不动** |
