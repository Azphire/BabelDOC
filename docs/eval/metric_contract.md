...
## 2f. 重放边界(batch-b9.5 会话一立;F3 改写并作废其例外条款)

第 1 节与 §2d 把"A/B 以缓存冻结重放为准"作为全部对照的前提。b9.5 为该前提写下一处
例外,F3 **作废该例外**,并把它替换为一条更强也更窄的条款。

### 作废的条款

> ~~修复环决策按设计绕缓存,为流水线唯一不可重放环节;跨跑差异按归因地板处置
> (E2.1 与 b9.4 两次实测)。~~

它当时是对的:`babeldoc/magazine/react/decide.py` 的 `EngineTransport` 以
`ignore_cache=True` 调用引擎,决策自带的缓存以**请求全文**为键,而请求全文含当次检
出的全部 finding 及其证据 —— 包括 `debug_id` 这种**每次运行重新分配**的字段。上游任
何一处差异都换掉键,决策重新采样。

**它当时站在两次实测上,两次都保留在案**:E2.1 实测 `gpt-4o` 在 `temperature=0`
下非确定(GAP-01);b9.4 实测同一份文档的三臂在同一个 finding 上作出不同决策,导致
一处未声明页的成品差异,按 GAP-20 的同一归因口径处置。两次都成立,而两次都**不**蕴
含"该环节不可重放"—— 它们蕴含的是"决策的键会变"。b10.2 修掉了键会变的成因之后,这
两次实测就只剩下第二条推论所说的那件事:引擎在**首次采样**时非确定。

### 作废的证据,三份,时间序

1. **b10.2 的 T3 修掉了成因。**键改取请求的一个**投影**:`issues_block` 渲染两次,
   一次是**发出去的**(逐字段不变),一次是**用来做键的**(丢掉
   `volatile_evidence_keys` 声明的字段:`debug_id`、`debug_ids`、
   `source_checkpoint`);两个请求点(决策轮与孤行翻译)走同一个
   `react/cache_key.py`,`CACHE_KEY_VERSION` 升到 2。**黑名单而非白名单**,理由写在
   模块里:键里还留着易变字段,代价是一次本该命中的未命中,它会出现在账上;键里丢
   了一个决定性字段,代价是命中了一个本不该命中的缓存,重放了一次针对别的证据作出
   的决策而没有任何地方说出这件事。实测:AramcoWorld-en-v2 重放 `api_calls` **0**、
   五轮决策全部 `from_cache: true` 且修复照样施加;Vogue-en **0**;CERNCourier-en
   **1**,而那一次是真正新出现的孤行**翻译**。出处
   `examples/output/b10_2/b10_2.report.md` §6。
2. **b10.4 的 R1 把"跨跑差异"的成因单独钉死。**同一份代码在
   AramcoWorld-en-v2 上跑两次:翻译请求文本 149 / 149,只在一边出现的 **0**;术语抽
   取请求 17 / 17,只在一边出现的 **0**;修复决策 5 轮 / 5 轮,全部命中;两次
   `api_calls` 皆 **0**。**请求集稳定则重放零成本**;b10.3 那次重放花掉的九次调用因
   此是"请求集不稳定",不是"环节不可重放"。出处
   `examples/output/b10_4/b10_4.report.md` §8。
3. **F3 的全量回归是这条结论的整语料检验。**六样张 41 页,全栈开启(含 b10.5 的栏级
   reflow),**请求 413,缓存命中 413,API 调用 0**;六份 `react_repair.report.json`
   的 `api_calls` 与 `api_attributions` 全为 **0**,而修复照常施加。这是本项目第一次
   有一整轮全语料运行**一次调用也没有付**。

### 现行条款

> 本流水线**不存在按设计不可重放的环节**。一次运行的可重放性等价于**它的请求集是否
> 稳定**:请求全文相同即命中缓存并按零成本重放;请求集变化即产生首见请求,并按次计
> 费。修复环的决策请求以请求全文的一个投影为键,投影丢弃
> `volatile_evidence_keys` 声明的逐运行字段,因此上游重新分配一个 `debug_id` 不再换
> 掉决策的键。跨跑差异仍按归因地板处置,但地板此后适用的命题是"**请求集为什么变
> 了**",不再是"该环节不可重放"。

三条推论,各自可断言:

- **推论 1(A/B 的预热臂)。**一次 A/B 若其第一臂产生任何首见请求,而该请求的答案进
  入下游文本,则两臂**不可比**,须先跑一次填缓存的预热跑。实测:b10.5 首轮 A/B 中
  AramcoWorld-en-v2 的 off 臂发出 1 次调用(自动术语抽取),其答案改变了下游批次文
  本,两臂译文在 6 页上不同;补跑之后两臂 `api_calls` 皆 0、译文逐字相同。**"一次跑
  完即可比"对该样张不成立**,而这不是不可重放。
- **推论 2(采样方差仍在,位置变了)。**`gpt-4o` 在 `temperature=0` 下非确定
  (GAP-01),那是**首次采样**的方差,不是重放的方差。一切显著性主张仍须三跑设计;本
  条改变的是"哪些差异需要解释",不是"差异可以不解释"。
- **推论 3(键的版本是证据的一部分)。**`CACHE_KEY_VERSION` 一升,旧键全部退休,**升
  版后的第一跑必然全部未命中**。任何引用"重放零成本"的数字都须同时说明它是在第几版
  键下取的。

### 2f.1 F3 的两臂运行协议(F3 追加)

带阶段计时的运行按两臂做,理由是**缓存温度是运行的属性而不是栈的属性**:

- **预热臂(cold)**:先跑一遍全语料,消化本栈的一切首见请求。它的计时**单独留档并
  标注**,不进任何靶单 —— 一个在等网络的 stage 不是一个慢的 stage。
- **计时臂(warm)**:预热之后再跑一遍,**只有这一臂的阶段计时可引**。

实测的温度差是这条协议存在的理由:同一 stack、同一输入的 Courier-en 单臂耗时在
**378s 与 55s** 之间,差别只在缓存温度与机器负载。若照冷臂定靶,靶单会指向错误的
stage。
