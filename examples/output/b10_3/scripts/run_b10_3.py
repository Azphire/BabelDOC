"""B10.3 driver: the five samples the paragraph granularity work is read on.

The stack is F2's stack plus the two passes that arrived after it -- the
parenthetical fold of B10.1 and the fragment stitch this batch adds -- so what
comes out is comparable with F2 page for page. Every sample is translated whole
rather than page ranged, for the reason B10.1 established: the requests a run
builds depend on how its pages group into articles and chains, so a page range
would build requests no earlier run built, and a request no earlier run built is
a request the project cache cannot answer.

Unlike B10.1 and B10.2 this batch does *not* claim a free replay, and says so
here rather than in a footnote. Three of its changes reach the request text.
Stitching joins pieces of a broken unit into one paragraph, so one request
replaces several. Record grouping cuts a contents entry set over several tight
lines into one paragraph rather than one per line, so one request replaces
several there too. And giving each record the style its own characters are set
in changes which style placeholders a request carries, because a placeholder is
written exactly where a run differs from the paragraph's base style. Every one
of those is a request the cache has never seen, and the ledger below records
what that cost, per sample, against the same sample's F2 figure.

What is written out is bounded: the pages of the batch's evidence table, as PNG
and as a PDF holding those pages alone, plus the sidecars the batch's mechanisms
are recorded in. The whole document stays in the working directory, which is not
what a reader of the batch is handed.

Beside those go two frozen comparisons against F2, written here rather than left
to be recomputed from the working directories, which are not tracked.
...
F2 produced. Paragraph counts are conserved by construction -- a stitch blanks
its members rather than removing them, and a split replaces one paragraph with
the records it held -- so what this file carries on a target page is either the
split account or nothing.

``parity.json`` is this run's requests set against F2's, request for request,
with the digest of every request text of each run and the requests whose text
changed. It covers the translator's own requests, which is what
``translate_tracking.json`` files under ``page``, ``cross_page`` and
``cross_column``; it does not cover the repair loop, which files its requests in
``react_repair.report.json`` under a scheme of its own. The record names both
the groups it read and the groups the tracking held, so a group appearing later
cannot be left out in silence.

Usage:
    python run_b10_3.py --all
    python run_b10_3.py --sample Vogue-en
"""
...
OUT_DIR = ROOT / "examples" / "output" / "b10_3"
...
def main(argv: list[str] | None = None) -> int:
    ...
    wanted = list(TARGETS) if args.all else (args.sample or ["Vogue-en"])
