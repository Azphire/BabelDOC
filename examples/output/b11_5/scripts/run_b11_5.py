"""B11.5 driver: one sample, one arm, with the batch's four repairs in force.

FD-en-v2 carries the evidence face of all four tasks -- the ruled glossary
entries of T1 and T4, the corner mark exemption of T2 and the indent policy of
T3 -- so one sample and one arm is the whole run, per CLAUDE.md section 4.14.

The switch set is read out of the f3 run record of the same sample rather than
retyped, as every driver since b7 has raised it, with this batch's new switch
added on top and recorded as added rather than folded in silently.

What this writes beyond the usual ledger is the batch's gate evidence. CLAUDE.md
section 4.16 asks a batch to extract, while it runs, the small derived files its
gate will read, instead of leaving the gate to open a checkpoint or a PDF. Two
are written here: render_evidence.json, every line of every page of the produced
document as position, size and text; and indent_evidence.json, the first line
offset of every paragraph as the typesetting stage left it. Both are JSON of a
few hundred kilobytes, which is inside archive_patterns and inside
archive_max_file_kb, so both survive the retention window in the archive.

Usage:
    python run_b11_5.py [--only SAMPLE ...]
"""
...
OUT_DIR = ROOT / "examples" / "output" / "b11_5"
...
# This batch's own switch, absent from the f3 record because it did not exist
# then. Raised here and recorded as raised, so the run's switch set says what it
# was rather than what it inherited.
NEW_SWITCHES = {"magazine_indent_policy": True}
...
def render_evidence(pdf: Path, sample: str, destination: Path) -> str:
    ...
    Position, size and text, rounded, one entry per line of each page. This is
    what CLAUDE.md section 4.16 asks a batch to extract while it runs: the gate
    asks about glyph sizes, line spacing and stray source text, and every one of
    those questions is answerable from here without opening the document again.
    """
    import pymupdf
...
def indent_evidence(working: Path, sample: str, destination: Path) -> str | None:
    ...
    The indent policy decides a flag and the typesetting stage turns that flag
    into an offset. Neither number is readable from the other, so both are
    recorded: the flag from the pass's own sidecar, and the offset measured here
    off the laid out document as the distance from the paragraph box to the
    first character of its first line.
    """
    ...
    for position, page in enumerate(document.page):
        ...
        for index, paragraph in enumerate(page.pdf_paragraph or ()):
            ...
            for composition in paragraph.pdf_paragraph_composition or ():
                # A laid out paragraph carries its characters one per
                # composition; an untouched one still carries them in a holder.
                # Both forms occur in this checkpoint, so both are read.
            ...
            boxed = [c for c in characters if c.box is not None]
...
def run_one(sample: str, layout_model) -> dict:
    ...
    record = {
        ...
        "switches": switches,
        "switches_added_this_batch": NEW_SWITCHES,
        "initial_adjacent_sha256": digest(
            ROOT / "configs" / "initial_adjacent.json"
        ),
