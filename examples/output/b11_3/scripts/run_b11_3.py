"""B11.3 driver: the regression set for the formula-font repair, on arm only.

The repair narrows one branch of the formula annotation, and it moves three font
names, all of them one sample's. Four of the five samples here are therefore a
control: they should come back matching the b10.5 on arm they answer to, and a
difference in them is a finding rather than an expectation.
...
Usage:
    python run_b11_3.py [--only SAMPLE ...]
"""
...
OUT_DIR = ROOT / "examples" / "output" / "b11_3"
...
# The regression set this batch's plan declares: five samples, one arm. The
# repair changes the annotation of one sample only -- it moves three font names
# and all three are FD's -- so four of the five are here to show that nothing
# moved where nothing should have. Courier-zh is left out on the same ground
# b11.2 left it out, and its annotation is measured offline in T2 and T3 all the
# same.
