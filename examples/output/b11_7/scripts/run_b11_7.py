"""B11.7 driver: six samples, one arm, with this batch's changes in force.

This is the settlement batch W-B11-12 called for: every change made since b11.1
is answered for on all six samples rather than on the subset each batch needed,
so the cross sample regression debt is closed here. Four changes are in force --
the indent policy is now the only writer of the first line flag, a body chain is
cut at its first box's capacity instead of at a sentence end, a formula
composition holding residue script is handed back to the translator, and a
rotated strip whose text was replaced is set along its own axis.
...
OUT_DIR = ROOT / "examples" / "output" / "b11_7"
BASELINE_BATCH = ROOT / "examples" / "output" / "b11_6"
...
COMPUTED_ATTRIBUTES = {
    ...
    formula_reclass.SWITCH: "magazine_formula_reclass",
    rotated_lane.SWITCH: "magazine_rotated_lane",
}

# Raised since the f3 record was written and therefore absent from it. Two of
# this batch's changes are new behaviour inside passes the switch set already
# turns on; two are new passes and bring a switch each.
SWITCHES_SINCE_F3 = {"magazine_indent_policy": True}
SWITCHES_THIS_BATCH = {
    "magazine_formula_reclass": True,
    "magazine_rotated_lane": True,
}
...
SIDECARS = (
    ...
    formula_reclass.REPORT_NAME,
    formula_reclass.RESTORE_REPORT_NAME,
    rotated_lane.REPORT_NAME,
...
# Every sample in the corpus. The settlement batch runs all of them.
REGRESSION_SAMPLES = (
    "Courier-en",
    "CERNCourier-en",
    "Vogue-en",
...
def baseline_switches(sample: str) -> dict:
    ...
    switches.update(SWITCHES_THIS_BATCH)
    return switches
...
def residue_evidence(pdf: Path, working: Path, sample: str, destination: Path) -> str:
    """Every character of the residue script the finished page still shows.

    The hard constraint this batch answers to is stated over the canvas -- no
    Chinese glyph anywhere in the text layer of a document finished into English
    -- so the count is taken from the produced PDF and not from the intermediate
    language. Both are recorded, because a difference between them is itself a
    finding: it would mean the detector and the page disagree about what is on
    the page.
    """
    import pymupdf
            ...
            for block in page.get_text("rawdict")["blocks"]:
                ...
                for line in block["lines"]:
                    for span in line["spans"]:
                        ...
                        counts = detector_base.script_counts(text)
                        ...
                        spans.append(
                            {
                                "page": index + 1,
                                "direction": [round(v, 2) for v in line["dir"]],
    ...
    record = {
        "sample": sample,
        "canvas_han_by_page": per_page,
        "canvas_han_total": sum(per_page.values()),
        "canvas_spans": spans,
        "il_paragraphs_with_han": il_rows,
        "il_han_total": sum(row["han"] for row in il_rows),
    }
    out = destination / "residue_evidence.json"
    out.write_text(
        json.dumps(record, indent=1, ensure_ascii=False) + "\n",
        encoding="utf-8",
        newline="\n",
...
def conservation(sample: str, working: Path, destination: Path) -> str | None:
    ...
    # b11.6 ran four of the six. The other two answer to their last full state,
    # which is f3; which one a sample was compared against is written down
    # rather than left to be inferred from whether a file happened to exist.
    baseline_path = BASELINE_BATCH / sample / "work" / sample / CHECKPOINT
    if not baseline_path.is_file():
        baseline_path = F3 / sample / "work" / sample / CHECKPOINT
    baseline = None
...
def run_one(sample: str, layout_model) -> dict:
    ...
    record = {
        ...
        "switches_added_this_batch": SWITCHES_THIS_BATCH,
        ...
        "residue_evidence": residue_evidence(final_pdf, working, sample, destination)
        if final_pdf
        else None,
