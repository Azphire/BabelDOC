"""B9.3 acceptance: line structure preservation against the stack, three arms.

Three arms over the three English samples whose declared pages carry records:
the contents page of Courier-en, the contents and masthead pages of FD-en-v2,
and the contents page of Vogue-en. All three arms run the finished stack; two of
them differ in one attribute, ``magazine_line_structure``, which is what the
batch is for.

The third arm is the control and repeats the first arm's configuration exactly.
It is here for the reason b9.2 established: this pipeline is not bit
reproducible -- a request the cache did not keep is asked again, the model is
sampled rather than looked up, and two runs of one configuration can disagree.
A difference between the first two arms counts against the switch only where
the control reproduced.

What the arm with the switch up pays for. The split turns one contents
paragraph into one paragraph per record, and the translator's unit is the
paragraph, so the same page is asked as more and shorter requests. None of them
is in the cache, because no arm ever asked them before. That is the honest cost
of the switch and it is recorded per arm rather than smoothed away.

``magazine_hitl_export`` is down for the reason it was down in F1 and b9.2:
export writes drafts into the user's directory, and a run that consumes a ruling
has no business rewriting the drafts beside it.

Usage:
    python run_b9_3_ab.py --arm off --all
    python run_b9_3_ab.py --arm control --all
    python run_b9_3_ab.py --arm on --all
    python run_b9_3_ab.py --arm on --sample Vogue-en
"""
...
OUT_DIR = ROOT / "examples" / "output" / "b9_3"
...
# The samples this batch accepts on: the ones whose page labels declare a kind
# the shipped vocabulary raises the line structure flag on. Named here rather
# than derived, because the acceptance is about these three layouts.
SAMPLES = ("Courier-en", "FD-en-v2", "Vogue-en")
...
# Switches that are not constructor parameters (W-B7-02, W-B8-01). The heading
# policy is up in every arm: it shipped with b9.2 and holding it down would make
# this acceptance about a stack nobody runs.
ATTRIBUTES = {
...
def run_one(sample: str, arm: str, layout_model) -> dict:
    ...
    setattr(config, line_split.SWITCH, ARMS[arm])
